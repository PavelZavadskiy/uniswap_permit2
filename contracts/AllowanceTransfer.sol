...
contract AllowanceTransfer is IAllowanceTransfer, EIP712 {
    ...
    /// @notice Зіставляє користувачів із маркерами на адреси споживачів та інформацію про схвалення на маркері
     /// @dev Індексується в порядку адреси власника маркера, адреси маркера, адреси споживача
     /// @dev Збережене слово зберігає дозволену кількість, термін дії дозволу та одноразовий термін
    ...
    /// @notice Внутрішня функція для передачі токенів за допомогою збережених дозволів
     /// @dev Не вдасться виконати, якщо дозволений часовий проміжок минув
    ...
    /// @notice Встановлює нові значення для суми, терміну дії та одноразового використання.
     /// @dev Перевірить, що підписаний nonce дорівнює поточному nonce, а потім збільшить значення nonce на 1.
     /// @dev Видає подію дозволу.
